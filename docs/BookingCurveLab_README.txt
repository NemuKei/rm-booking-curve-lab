...
1. config/hotels.json を開き、ホテル情報を確認
   例：
   {
     "daikokucho": {
       "display_name": "なんば大国町",
     ...
     "kansai": {
       "display_name": "ホテル関西",
       "data_subdir": "hotel_kansai",
       "capacity": 434.0,
       "forecast_cap": 416.0
     }
   }

2. data/<data_subdir>/ に PMS の時系列データ Excel を配置
   - シート名：202512 のように宿泊月の YYYYMM
   - 行列の形式は現状のテンプレに揃える（先頭行：取得日、先頭列：宿泊日）

3. アプリを起動し、「マスタ設定」タブでホテル・予測キャップ・稼働率キャパが想定どおりか確認・保存


5. 日常運用フロー（ざっくり）
--------------------

5-1. 月次更新（新しい月のデータを追加したとき）

1. PMS から最新の時系列オンハンドデータを出力し、該当ホテルの Excel に追記／更新
2. BookingCurveLab を起動
3. 「ブッキングカーブ」タブで対象ホテルを選択
4. 「LT_DATA(4ヶ月)」または「LT_DATA(期間指定)」ボタンを押す
   → lt_data_YYYYMM_*.csv と monthly_curve_YYYYMM_*.csv が同時に再生成される
5. 「月次カーブ」タブで、最新月・前年同月のカーブを確認
...
7-4. RMSE（Root Mean Squared Error）

- 定義：
  - 各日の誤差を二乗して平均し、その平方根を取ったもの
- 単位：室数（rooms）
- 特徴：
  - 大きな外しに敏感（事故的な日があると数値が跳ねる）
  - 「1日あたり何室ぶん外しているか」の感覚で読める
- 解釈：
  - RMSE = 20 → 「平均すると、1日あたり 20室くらい外しているイメージ」
- 一言：
  - 「RMSE は“大きなミス”に敏感な指標。1日あたり何室ぶん外しているかを見る。」
