...
history_months = r.get_history_months_around_asof(as_of_ts=asof_ts, months_back=4, months_forward=4)
...
def summarize_detail(tag: str, detail: pd.DataFrame):
    ...
    print(tag, f"detail_rows={len(detail)} gated_true={gated_true} factor!=1={neq1}")
    if "factor" in cols:
        f = detail["factor"].astype(float)
        print(tag, "factor stats:", "min=", float(f.min()), "p50=", float(f.median()), "max=", float(f.max()))
...
def run(boundary: str):
    ...
    print("\n=== boundary =", boundary, "===")
    print("forecast_sum=", float(fc_n.sum(skipna=True)), "len=", len(fc_n), "na=", int(fc_n.isna().sum()))
...
print("LT_MIN/LT_MAX=", r.LT_MIN, r.LT_MAX, "RECENT90_MIN_COUNT_WEEKDAY=", r.RECENT90_MIN_COUNT_WEEKDAY)
...
print("factor_map common=", len(common), "diff_in_common=", int(diff_keys), "iso_only=", len(set(iso_map)-set(sun_map)), "sun_only=", len(set(sun_map)-set(iso_map)))
