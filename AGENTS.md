...
## -1. 最優先ルール（憲法 / Quick Rules）

1. 迷ったら **`docs/spec_*.md` と `AGENTS.md` を唯一の正**として参照し、書いていないことは推測で断定しない。
2. 判断材料（対象コード／仕様／入出力例／再現手順）が不足している場合、**推測で進めず要求**し、揃うまで壁打ちに切り替える。
3. 参照の唯一の正は **この依頼（またはスレッド）で共有された「アンカーZIP」**（`make_release_zip.py` で作成）。作業中に参照点を勝手に動かさない。
4. 仕様とコードが矛盾して見える場合は **仕様優先**。不明点は質問・TODO・Decision候補として明示する。
5. docs運用：`docs/spec_*` は原則人間＋ChatGPTが編集し、Codex は read-only（例外は明示指示がある場合のみ）。
6. Thread Log / Decision Log は「作業記録／決定の記録」であり、**仕様の代替にしない**（仕様は spec_*）。
7. Codex依頼は **定型フォーマット**を厳守（短い説明＋コメントのみブロック、参照spec・タスクID/スコープ・完了条件を含める：詳細は Section 7）。
...
- 共有ファイルは **`make_release_zip.py` で作成したアンカーZIP（この依頼/スレッドで共有されたZIP）** を唯一の正とし、手作業の単体ファイル添付は原則しない。
...
### 7.1 プロンプトの構造（固定フォーマット）

Codex に渡すプロンプトは、**必ず次の2パート構成**にする（例外なし）。

1. **短い説明パート（1〜3行）**
   - どのファイルを開くか
   - 置換範囲（ファイル全体／特定関数／特定ブロック）
   - 下のプロンプトを貼ること

2. **Pythonコメントのみの大きなコードブロック**
   - コードブロック内は **すべて `#` から始まるコメントのみ**（コードを書かない）
   - 差分ではなく、原則 **修正後ファイル全体**を出力させる（置換前提）

### 7.2 コメントブロックに必ず含める要素（順序推奨）

1. 役割宣言
2. 対象ファイルのパス
3. 目的（What & Why）
4. 前提・制約（変えてはいけない外部仕様、入出力、公開API等）
5. 具体的な修正内容（番号付き）
6. 出力の指示（「修正後ファイル全体」「Pythonコードのみ」等）
7. 参照すべき仕様ファイル（`docs/spec_*.md`）
8. タスクIDとスコープ（今回触る範囲を固定）
9. 完了条件（互換性、既存テスト、既存CSV互換等）
...
11. 共有の唯一の正（アンカーZIP）を無視して、断片的な単体ファイルだけで判断を進めること
