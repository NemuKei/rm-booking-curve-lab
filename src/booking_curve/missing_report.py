...
from booking_curve.pms_adapter_nface import _parse_nface_filename
...
def discover_raw_nface_files(input_dir: Path, recursive: bool = True) -> dict[tuple[str, str], Path]:
    """
    ...
    for path in globber("*.xls*"):
        ...
        target_month, asof_date = _parse_nface_filename(path)
...
def _build_snapshot_missing_records(df: pd.DataFrame, hotel_id: str, daily_snapshots_path: Path) -> list[dict[str, object]]:
    records: list[dict[str, object]] = []
    df_filtered = df.copy()
    ...
    for (asof_ts, target_month), group in df_filtered.groupby(["as_of_date", "target_month"]):
        expected_dates = iter_month_dates(target_month)
...
def build_missing_report(
    ...
    *,
    ...
    raw_files = discover_raw_nface_files(input_dir, recursive=recursive)
    ...
    if not daily_snapshots_path.exists():
        ...
        records.extend(_build_snapshot_missing_records(df_snapshots, hotel_id, daily_snapshots_path))
