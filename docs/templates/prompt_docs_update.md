...
## 目的
この依頼に添付されたアンカーZIP（作業開始点）を根拠に、指定された docs を **参照齟齬なく** 更新する。
（spec_* が唯一の正。Thread Log / Decision Log との整合も確認する）

---

## 運用ルール（必ず遵守）
- 共有物の唯一の正：この依頼に添付されたアンカーZIP（作業開始点のZIP。作業中に参照点を動かさない）
...
## 入力（貼り付ける）
- アンカーZIP名（この依頼に添付されたZIP）：`<<<ここにZIPファイル名>>>`
- Branch：`<<<例: main>>>`
- Commit：`<<<例: 265cb5b>>>`
...
- 仕様（spec_*）と実装がズレている場合：
  - 原則：spec_* が唯一の正。乖離がある場合は **(a) specを正として実装を直す** か **(b) 仕様変更としてDecisionを起こしてspecを更新** のどちらかを明示し、勝手に片方へ寄せない

---

## 出力（厳守）
### Output Mode（どちらかを選ぶ）
原則は Mode-A（全文出力）。ただし、ユーザーが「パッチで欲しい」「git apply で当てたい」等を明示した場合は Mode-B を使ってよい。

#### Mode-A（既定）：更新後の対象ファイル全文
- 更新後の **対象ファイル全文**を出力する（差分ではなく全文置換前提）
- 余計な解説は出さない（ファイル本文のみ）
- 変更した箇所は、最低限の範囲で反映（無関係な整形は禁止）

#### Mode-B（任意）：Unified diff パッチ（機械適用）
- 出力は **単一の unified diff** のみ（余計な解説は禁止）
- 対象は「更新対象ファイル」だけに限定（他ファイルを巻き込まない）
- 変更は最小差分（無関係な整形は禁止）
- 目的：人間（または明示許可されたツール）が `git apply` / `apply_patch` 相当で **機械的に適用**してブレを無くす