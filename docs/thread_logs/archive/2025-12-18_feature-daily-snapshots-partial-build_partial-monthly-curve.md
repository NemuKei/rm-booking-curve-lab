# ファイル名案: docs/thread_logs/2025-12-18_feature-daily-snapshots-partial-build_partial-monthly-curve.md

# Thread Log: daily snapshots 部分更新と月次カーブ整合

## Meta

* Date: 2025-12-18（ユーザー指定：引継書作成日）
...
* 「前年データが無いとエラー」挙動は改善方向（取れる月だけ警告→描画、ゼロ件のみエラー）で合意
...
* Decision:
  ...
  * FULL_ALL は「普段触らない奥」＋実行前見積り表示＋強制ログ保存＋最終確認で誤爆防止
...
* Why:
  ...
  * monthly_curve を LT_DATA から代用すると定義が崩れて誤った可視化になるため、フォールバックは禁止・統一が合理的

## Docs impact（反映先の候補）

* spec_overview: daily_snapshots 更新モード（partial/fast/full）の運用整理、monthly_curve生成と描画の優先順位
* spec_data_layer: daily snapshots 部分生成（Partial/Upsert）の運用定義、monthly_curve生成の正しいルートとフォールバック禁止
* spec_models: （影響小：モデル自体は未変更）
* spec_evaluation: （影響小：評価ロジック自体は未変更）
* BookingCurveLab_README: GUI操作（FAST/FULL_MONTHS/FULL_ALL）と注意点（見積り/ログ/誤爆防止）追記候補

### Docs impact 判定（必須/不要 + Status）

* 必須:

  * 項目: daily_snapshots 更新モード（FAST / FULL_MONTHS / FULL_ALL）の位置づけと範囲定義（ASOF窓=latest-14d、FULL_MONTHSはASOF窓なし）

    * 理由: 週次運用手順がブレると誤更新・更新漏れ・誤爆が起きる
    * Status: 未反映
    * 反映先: docs/spec_data_layer.md（daily snapshots build modes / partial build 節）
  * 項目: monthly_curve の生成・描画の優先順位（monthly_curve無→daily_snapshotsから生成→保存→描画）と LT_DATAフォールバック禁止

    * 理由: 誤った曲線表示が意思決定を壊すため、仕様として禁止・固定が必要
    * Status: 未反映
    * 反映先: docs/spec_data_layer.md（monthly_curve 生成ルート・欠損時挙動の節）
  * 項目: FULL_ALL の誤爆防止（奥配置・最終確認・見積り表示・強制ログ保存）

    * 理由: 全量再生成は負荷/時間/誤操作リスクが高い
    * Status: 未反映
    * 反映先: BookingCurveLab_README.txt（運用/注意事項）
* docs不要:

  * 項目: WinError5（ファイルロック）によりPermissionErrorが出る

    * 理由: 仕様ではなく運用Tips。必要ならREADMEの注意事項で十分
  * 項目: 509ファイル=約20秒という実測値

    * 理由: 環境依存で仕様固定に不向き（見積り実装の参考に留める）

## Known issues / Open

* 月次カーブ描画時、monthly_curve欠損時の「その場生成」ロジックが別系統（LT_DATA経由など）になっている疑いがあり、曲線が崩れる
* monthly_curve の欠損補完（ASOF欠けで1点抜け）が本来仕様と一致していない可能性（要精査）
* 「前年が無いとエラー」挙動は改善方向に寄っているが、完全に意図通りかは要再確認

## Next

* P0:

  * monthly_curve 生成ロジックを一本化し、monthly_curve無時は daily_snapshots から生成→CSV保存→描画に統一
  * daily_snapshots が無い/薄い場合の月次カーブ挙動（エラー文言と更新誘導）を確定
* P1:

  * 月次カーブ描画：取れる月が1本でもあれば警告→描画、1本も無い場合のみエラーに統一
  * FULL_ALL 後の後続処理（全期間LT生成のY/Nなど）導線を最終決定（monthly_curveは月次タブで都度生成でも可）
