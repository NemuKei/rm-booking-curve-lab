...
- 実行時の保存先（重要）：
  ...
  - Windows の既定：`%LOCALAPPDATA%/BookingCurveLab/`
    ...
    - `output/`（CSV・ログ）
    - `acks/`（欠損検査 ops の確認済み）
    - `local_overrides/`（端末ローカル上書き：RAW取込元の上書き＋GUI設定を含む）
- ログ：
  - `output/logs/gui_app.log` を標準の実行ログとする（回転ログ）。
  - エラー発生時は、GUIで「ログフォルダを開く」導線を提供する。
- 管理者・開発者：
...
- GUI から見た挙動：
  ...
  - 欠損値の補完（NOCB）はまだ実装していない（次フェーズで対応）。
  - ただしビュー側では、月次カーブ等で **線形補完（欠損補完=線形）** を任意で適用できる。
    （データレイヤーのCSVは生のNaNを保持し、補完は表示用DataFrameでのみ行う）
...
- **欠損値補完ポリシーを整理（データレイヤーは生の NaN を保持）。**
  ...
  - グラフ描画・評価で参照する「ビュー用 DataFrame」に対して、LT方向の補完を適用する。
    - 既定は「LTが若い方向へ進む」ことを時系列とみなし、**LOCF（Carry Forward）** を採用。
    - 月次カーブなど一部ビューは、任意で **線形補完（欠損補完=線形）** を選択できる（データレイヤーのCSVは変更しない）。
    - 例外：月次カーブで `LT=0` が欠損し `LT=-1 (ACT)` が存在する場合、線形補完の起点確保のため **ビュー上でのみ** `LT=0 <- ACT` をコピーしてから補完する。
